...
- Fixed a deeper, still-latent build bug found while re-verifying via the actual `yarn run build`/`buildNextApp()` pipelines (not just a direct `next build` from inside `client/`): both resolved `next` to **root's** separate `next` installation instead of `client/`'s own, because root and `client/` each have their own independent `node_modules`. Next's SSR runtime does plain `require("react")` calls that bypass webpack aliasing, so root's `next` pulled in root's copy of React alongside `client/`'s, crashing every prerendered page with `TypeError: Cannot read properties of null (reading 'useContext')`. This affected both the root `build` npm script and `scripts/nextBuild.js` (the production fallback `server/index.js` runs when `client/.next` is missing) — the latter is a real prod-path bug that predates this session, just never exercised because the build always failed earlier at the type-check stage first. Fixed by making both explicitly invoke `client/node_modules/.bin/next` instead of relying on ambiguous `next` resolution; `scripts/nextBuild.js` now spawns it as a child process instead of calling `next/dist/build/index.js` in-process.
